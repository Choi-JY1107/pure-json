<script lang="ts">
	...
	function handleConvert() {
		try {
			editor.setOutput(jsonToTypescript(editor.input));
		} catch (e) {
...
<MetaTags
	title={m.to_typescript_title()}
	description={m.to_typescript_description()}
/>

<div class="tool-page">
	<div class="tool-page__toolbar">
		<h1 class="tool-page__title">{m.to_typescript_h1()}</h1>
	...
	<div class="tool-page__grid">
		...
		<div class="tool-page__panel">
			<div class="tool-page__label-row">
				<span class="tool-page__label">{m.typescript_output_label()}</span>
				<CopyButton text={editor.output} />
			</div>
			<div class="tool-page__editor">
				<MonacoEditor value={editor.output || editor.error} readOnly language="typescript" />
			</div>
		</div>
	</div>
	...
	<article class="tool-page__article prose prose-sm">
		<h2>{m.to_typescript_h1()}</h2>
		<p>{m.to_typescript_intro()}</p>
		<details>
			<summary><strong>{m.to_typescript_faq_q1()}</strong></summary>
			<p>{m.to_typescript_faq_a1()}</p>
		</details>
		<details>
			<summary><strong>{m.to_typescript_faq_q2()}</strong></summary>
			<p>{m.to_typescript_faq_a2()}</p>
		</details>
	</article>
</div>
